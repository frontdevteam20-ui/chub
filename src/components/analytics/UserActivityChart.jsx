  ...
  useEffect(() => {
    async function fetchUserActivitySummary() {
      try {
        ...
        // Set stats based on view mode
        if (viewMode === 'year') {
          setStats([
            { label: "Year Total", value: totalUsers, color: "blue", icon: "📅", trend: `${trendSign}${trendPercentage}%` },
          ]);
        } else {
        ...
        console.error('❌ User Activity Chart fetch error:', err);
        setChartData([["Date", "Active Users"]]);
        setStats([]);
      } finally {
        setLoading(false);
